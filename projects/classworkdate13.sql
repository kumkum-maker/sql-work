...
select customer_id,customer_name,amount,sum(amount) over(rows between 2 preceding and current row ) from orders;
select customer_id,customer_name,amount,sum(amount) over(order by amount rows between unbounded preceding and current row ) as result 
from orders;
select order_id,customer_id,customer_name,order_date,amount,sum(amount) over
(partition by customer_id order by order_date rows between 2 preceding and current row)
from orders;
select order_id,customer_id,customer_name,order_date,amount,sum(amount) over
(partition by customer_id order by order_date rows between 1 preceding and 1 following)
from orders;
